...
class SustainabilityNormalizer:
    ...
    Updated to handle new indicator mappings
    """
    ...
    def __init__(self, db_file_path='sustainability_data.db'):
        ...
        # NEW: Indicator mapping (old -> new)
        self.indicator_mapping = {
            # Society -> People mappings
            'Population': 'Health',
            'ArtsAndCulture': 'Health',
            'Arts and Culture': 'Health',
            'Community': 'Community',
            'Education': 'Education',
            'Wealth': 'Wealth',
            
            # Economy -> Prosperity (stays same)
            ...
            # Environment -> Planet mappings
            'Biodiversity': 'Biodiversity',
            'Land': 'Land, Air & Water',
            'Air': 'Land, Air & Water',
            'Water': 'Land, Air & Water',
            'ClimateChange': 'Built Environment',
            'Climate Change': 'Built Environment',
            
            # NEW: Food & Agriculture Systems (if you have this data)
            'FoodAndAgriculture': 'Food & Agriculture Systems',
            'Food': 'Food & Agriculture Systems',
            'Agriculture': 'Food & Agriculture Systems',
        }
        
        # Define metrics where "lower is better" (these need reversed percentiles)
        self.reverse_metrics = {
            # Health metrics (lower is better) - Updated from Population
            ...
            # Environmental metrics (lower is better) - Updated paths
            'Planet_BuiltEnvironment_Co2PerCapita2021PerStateAverageMetricTons',
            'Planet_Land, Air & Water_AirQualityIndexPerPm2.5',
            'Planet_Land, Air & Water_WaterConservationOrGallonsOrPersonOrDay'
        }
    
    def remap_indicators(self):
        """Remap old indicator names to new indicator names"""
        print("🔄 Remapping indicators to new structure...")
        
        cursor = self.conn.cursor()
        
        # Get distinct sub_measures that need remapping
        cursor.execute("""
            SELECT DISTINCT sub_measure, COUNT(*) as count
            FROM raw_metrics
            WHERE sub_measure IS NOT NULL
            GROUP BY sub_measure
        """)
        
        old_indicators = cursor.fetchall()
        remapped_count = 0
        
        for old_name, count in old_indicators:
            # Check if this indicator needs remapping
            new_name = self.indicator_mapping.get(old_name)
            
            if new_name and new_name != old_name:
                # Update the raw_metrics table
                cursor.execute("""
                    UPDATE raw_metrics
                    SET sub_measure = ?
                    WHERE sub_measure = ?
                ...
                remapped_count += cursor.rowcount
                print(f"   ✅ Remapped '{old_name}' → '{new_name}' ({count} metrics)")
            else:
                print(f"   ℹ️  Keeping '{old_name}' (no mapping needed)")
        
        self.conn.commit()
        print(f"   Total records remapped: {remapped_count}")
    ...
    def generate_normalization_summary(self):
        ...
        # Show remapped indicators
        print(f"\nRemapped Indicators:")
        indicators_query = """
            SELECT DISTINCT sub_measure, COUNT(*) as metric_count
            FROM raw_metrics
            WHERE sub_measure IS NOT NULL
            GROUP BY sub_measure
            ORDER BY sub_measure
        """
        indicators_df = pd.read_sql(indicators_query, self.conn)
        for _, row in indicators_df.iterrows():
            print(f"  {row['sub_measure']}: {row['metric_count']} metrics")
    ...
    def run_stage2(self):
        ...
        try:
            ...
            # Step 0: Remap indicators to new structure (NEW STEP)
            remapped = self.remap_indicators()
            ...
            print(f"   Indicators remapped: {remapped}")
...
if __name__ == "__main__":
    ...
    print("\n🎯 Next Steps:")
    print("   - Run stage2_verification_updated.py to verify the remapped data")
    print("   - Update enhanced_radar_V2_with_fast_state.py with new indicator names")
    print("   - Update county_secure_dashboard.py to display new structure")
    print("   - Test the visualization with remapped indicators")
